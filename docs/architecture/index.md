...
- YouTube video ads are pruned at the source by a MAIN-world script that strips `adPlacements`/`adSlots`/`playerAds` from Innertube JSON on every path in — inline `ytInitialPlayerResponse`/`ytInitialData`, fetch and XHR responses for the ad-carrying endpoints, and same-origin iframe realms — so the real video plays with no pre/mid-roll; skip-button clicks and fast-forward remain as a safety net, and playback resumes on its own when an ad ends.
- Pop-up/pop-under defusing: a MAIN-world script wraps `window.open` and blocks the abusive pattern (cross-origin opens from clicking non-interactive areas, gesture-less opens, floods) while allowing legit pop-ups (OAuth, share); blocked calls get a decoy window so the site doesn't retry.
