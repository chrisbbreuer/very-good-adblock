...
try {
  ...
  await popup.click('#page-blocks-toggle')
  await waitFor(popup, `document.querySelectorAll('#page-blocks .page-block-row').length === 4`, 'page blocks list rows')
  await waitFor(popup, `[...document.querySelectorAll('#page-blocks .page-block-label')].some(el => el.textContent?.includes('ads.example.test'))`, 'page blocks list labels')
  await assertNoHorizontalOverflow(popup, 'popup blocks list')
  await screenshot(popup, 'popup-blocks.png')
  await popup.click('#page-blocks-toggle')
  await waitFor(popup, `document.querySelector('#page-blocks')?.hidden === true`, 'page blocks collapse')
  ...
  await waitFor(options, `document.querySelectorAll('#recent-blocks .diagnostic-row').length === 5`, 'recent blocks rows')
  await waitFor(options, `[...document.querySelectorAll('#recent-blocks .diagnostic-row span')].some(el => el.textContent?.includes('example.test'))`, 'recent blocks hosts')
