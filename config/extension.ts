...
• Consistent typography across the popup and dashboard on every device.`,
    ...
    // Generated by `buddy generate:app-store` from `config/images.ts`, which
    // `extension:safari:publish` runs on the way to the upload — so the set
    // that ships describes the build that ships. The names are the generator's
    // (`<display-type>-<nn>.png`), numbered per device class in slide order.
    //
