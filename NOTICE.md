...
Generated host rules are committed in `src/rules/generated/network-hosts.json`. They are produced by `bun run update:filters` from exact immutable upstream Git revisions listed in `src/rules/filter-sources.json`; each generated source entry includes the raw URL, revision, license label, host count, and SHA-256 of the fetched source text.
